        ...
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="availability d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        <a href="/student/enrollnow/{{ $tutor_profile_id }}"><button class="btn btn-sm btn-primary">Book
                                More Slots</button></a>
                    </div>
                </div>
                ...
                <form id="payment-search" action="{{ route('student.updateslots') }}" method="POST">
                    ...
                    <div class="row ">
                        ...
                        <?php
                        $class_purchased = paymentstudents::where('student_id', session('userid')->id)->where('class_id', $enrollment->class_id)->where('tutor_id', $enrollment->tutor_id)->where('subject_id', $enrollment->subject_id)->sum('classes_purchased');
                        ?>


                        <div class="col-md-2 mt-4">
                            <label for="">Total Classes</label>
                            <input type="number" class="form-control readonly" name="classpurchased" id="classpurchased"
                                value="{{ $class_purchased }}" readonly>
                            <span class="text-danger">
                        ...
                        <div class="col-md-2 mt-4">
                            ...
                            <input type="text" class="form-control readonly" name="slotspending"
                                value="{{ $class_purchased - $bookedSlotsCount }}" id="slotspending" readonly>
                            <span class="text-danger">
                                @error('slotspending')
                                    {{ $message }}
                                @enderror
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped align-middle table-nowrap mb-0 users-table"
                            style="display:block; overflow-Y:scroll !important;">
                            ...
                            <tbody>
                                                ...
                                                @foreach ($slots as $slot)
                                                    @php
                                                        ...
                                                        if ($slot['bookedbyme']) {
                                                            ...
                                                            $buttonClass .= $slot['is_available']
                                                                ? 'btn-success'
                                                                : 'btn-danger';
                                                            $buttonClass .=
                                                                $slot['is_available'] == 1 ? ' btn-primary' : '';
                                                        }
                                                    @endphp
                                                    <button type="button" class="slot-btn btn btn-sm {{ $buttonClass }}"
                                                        data-date="{{ $date }}" data-time="{{ $slot['time'] }}"
                                ...
                                @else
                                    <div class="alert alert-danger mt-3">
                                        ...
                                        <strong>Update Booking Unavailable:</strong> This tutor has not set their
                                        availability slots yet.Update Enrollment is disabled until slots are created.
                                    </div>
                                @endif
                ...
                <div style="display: flex; justify-content:space-between" class="my-3">
                    @if (isset($groupedSlots) && count($groupedSlots) > 0)
                        ...
                        <div>
                            ...
                            <span>
                                <label for="contactadmin">
                                    Please select to contact Admin, if you are not able to select slots.
                                </label>
                            </span>
                        </div>
                    @endif
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.slot-btn').on('click', function() {
                if (parseInt(document.getElementById('slotspending').value) < 1 || document.getElementById(
                        'slotspending').value == '') {
                ...
                if (selectedSlots.length >= parseInt(document.getElementById('slotspending').value)) {
                    alert('You already selected ' + selectedSlots.length +
                        ' as per your required classes.');
                    return false;
                }
                var $button = $(this);
